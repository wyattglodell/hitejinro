<?php
	$admin->set_keyword('News');
	$admin->set_tbl($conf->NEWS);
	$admin->set_primary_key('news_id');

	$admin->allow_add(1);
	$admin->allow_edit(1);
	$admin->allow_delete(1);	
	
	$admin->set_ordering('news_dt DESC');
	...
	*/
	...
	$admin->thumbnail('tiny','fixed',200,200); # thumbnail_name, aspect | fixed | crop, width, height. for aspect: one side is optional, pass 0
	
	$admin->add_field('site','Site', '100|center', $conf->sites);
	$admin->add_field('news_dt','Date', '50|center|date');
	...
	$admin->add_search('site', 'Site', 'select', $conf->sites);		
